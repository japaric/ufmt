...
impl uDebug for u8 {
    ...
    where
        W: uWrite + ?Sized,
    {
        let mut buf = unsafe { mem::MaybeUninit::<[u8; 3]>::uninit().assume_init() };
...
impl uDebug for u16 {
    ...
    where
        W: uWrite + ?Sized,
    {
        let mut buf = unsafe { mem::MaybeUninit::<[u8; 5]>::uninit().assume_init() };
...
impl uDebug for u32 {
    ...
    where
        W: uWrite + ?Sized,
    {
        let mut buf = unsafe { mem::MaybeUninit::<[u8; 10]>::uninit().assume_init() };
...
impl uDebug for u128 {
    ...
    where
        W: uWrite + ?Sized,
    {
        let mut buf = unsafe { mem::MaybeUninit::<[u8; 39]>::uninit().assume_init() };
